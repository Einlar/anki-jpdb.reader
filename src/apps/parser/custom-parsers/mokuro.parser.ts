...
export class MokuroParser extends AutomaticParser {
  ...
  protected override setupVisibleObserver(): void {
    /* noop – disabled for Mokuro custom logic */
  }
  protected override setupAddedObserver(): void {
    /* noop – disabled for Mokuro custom logic */
  }
  protected override addedObserverCallback(_nodes: HTMLElement[]): void {
    /* noop – disabled for Mokuro custom logic */
  }
  ...
  private executeParse(panel: HTMLElement): void {
    ...
    const validContainers = Array.from(pageContainers).filter(
      (container) => container.querySelector('.textBox') !== null,
    );
    ...
    try {
      ...
      validContainers.forEach((container) => {
        Registry.batchController.registerNode(container, {
          ...
          applyFn: (paragraph: Paragraph, tokens: JPDBToken[]): void => {
